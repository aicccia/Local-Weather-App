...
function getCurrentWeatherData(lat, lon, temperatureSystemIsF) {
	...
	$.getJSON(url, (weather) => {
		...
		$('#cityName').css('opacity', 0.2).empty().append(`<p> ${city}, ${country} </p>`)
			.animate({opacity: 1}, 1000);
...
function getAndUpdateForecastDetail() {
	...
	const url = `https://api.openweathermap.org/data/2.5/forecast?lat=${latlog[0]}&lon=${latlog[1]}&appid=4bd0296ac3468ba55671920cabb0f745`;
